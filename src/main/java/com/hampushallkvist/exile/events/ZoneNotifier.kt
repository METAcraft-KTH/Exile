...
class ZoneNotifier : Event {
    ...
    companion object {
        ...
        fun getPlayerZone(serverPlayerEntity: PlayerEntity): Zone {
            if (serverPlayerEntity.pos.z > 0) return Zone.CIVILIZATION
            return Zone.WILDERNESS
        }

        fun updatePlayerZone(
                storage: DataCommandStorage,
                serverPlayerEntity: PlayerEntity,
                newZone: Zone
        ) {
            val exileNamespace = storage.get(Identifier("exile", "exiles"))
            val currentPlayerNbt =
                    exileNamespace.getCompound(serverPlayerEntity.entityName) ?: return
        ...
        server.playerManager.playerList.forEach {
            ...
            if (player != null && currentZone != null) {
                ...
                if (player.pos.z < 0 && currentZone == Zone.CIVILIZATION.toString()) {
                    ...
                    player.sendMessage(
                            LiteralText("You have entered wilderness, be alert!")
                    ...
                    player.sendMessage(
                            LiteralText("You have entered civilization, you are now safe")
                                    .formatted(Formatting.GOLD),
                            false
                    )
                }
